...
private:
	...
	struct ClientShadow_t
	{
		...
		Vector					m_LastOrigin;
		QAngle					m_LastAngles;
		//Vector					m_CurrentLightPos;
		//Vector					m_TargetLightPos;
		//float					m_LightPosLerp;
...
void CClientShadowMgr::InitDepthTextureShadows()
{
	...
	// SAUL: start benchmark timer (env_projectedtexture)
	CFastTimer timer;
	timer.Start();
...
void CClientShadowMgr::BuildFlashlight( ClientShadowHandle_t handle )
{
	...
	// Don't project the flashlight if the frustum AABB is not in our view (env_projectedtexture)
	Vector mins, maxs;
