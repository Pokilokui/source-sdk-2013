...
public:
	...
	//Addition due to Client Renderable
	matrix3x4_t z;
...
int	C_BeamQuadratic::DrawModel(int)
{
	...
	CMatRenderContextPtr pRenderContext(materials); //Addition
	...
	for (int i = 0; i < 3; i++)
	{
		dl[i] = effects->CL_AllocDlight(m_viewModelIndex + i);
		dl[i]->origin = points[i];
		dl[i]->color.r = 159;
		dl[i]->color.g = 255;
		dl[i]->color.b = 255;
