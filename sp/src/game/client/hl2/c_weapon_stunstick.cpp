...
//
// Purpose: Client side code weapon_stunstick
// Code fixed by Mazdo
...
void C_WeaponStunStick::DrawFirstPersonEffects(void)
{
	...
	// Dynamic lighting for the stunstick for when you use it
	if (InSwing())
	{
		Vector vAttachement;
		QAngle tempAng;
		GetAttachment("Sparkrear", vAttachement, tempAng);

		dlight_t* dl = effects->CL_AllocDlight(index);
		dl->origin = vAttachement;
		dl->color.r = 245;
		dl->color.g = 245;
		dl->color.b = 245;
		dl->color.exponent = -1;
...
void C_WeaponStunStick::StartStunEffect(void)
{
	// Play startup sound
}

void C_WeaponStunStick::StoptStunEffect(void)
{
	// Play shutdown sound
}
