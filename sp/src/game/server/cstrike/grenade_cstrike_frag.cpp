...
#include "grenade_cstrike_frag.h"
#include "Sprite.h"
...
ConVar sk_plr_dmg_cstrike_fraggrenade("sk_plr_dmg_cstrike_fraggrenade", "0");
ConVar sk_npc_dmg_cstrike_fraggrenade("sk_npc_dmg_cstrike_fraggrenade", "0");
ConVar sk_cstrike_fraggrenade_radius("sk_cstrike_fraggrenade_radius", "0");

#define GRENADE_MODEL "models/Weapons/w_eq_fraggrenade.mdl"

class CGrenadeCstrikeFrag : public CBaseGrenade
{
	DECLARE_CLASS(CGrenadeCstrikeFrag, CBaseGrenade);

#if !defined( CLIENT_DLL )
	DECLARE_DATADESC();
#endif

	~CGrenadeCstrikeFrag(void);
...
LINK_ENTITY_TO_CLASS(npc_grenade_cstrike_frag, CGrenadeCstrikeFrag);

BEGIN_DATADESC(CGrenadeCstrikeFrag)
...
// Purpose: 
//-----------------------------------------------------------------------------
CGrenadeCstrikeFrag::~CGrenadeCstrikeFrag(void)
{
}

void CGrenadeCstrikeFrag::Spawn(void)
{
	Precache();
	...
	{
		m_flDamage = sk_plr_dmg_cstrike_fraggrenade.GetFloat();
		m_DmgRadius = sk_cstrike_fraggrenade_radius.GetFloat();
	}
	else
	{
		m_flDamage = sk_npc_dmg_cstrike_fraggrenade.GetFloat();
		m_DmgRadius = sk_cstrike_fraggrenade_radius.GetFloat();
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeCstrikeFrag::OnRestore(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
/*void CGrenadeCstrikeFrag::CreateEffects(void)
{
...
bool CGrenadeCstrikeFrag::CreateVPhysics()
{
...
void CGrenadeCstrikeFrag::VPhysicsUpdate(IPhysicsObject* pPhysics)
{
...
void CGrenadeCstrikeFrag::Precache(void)
{
...
void CGrenadeCstrikeFrag::SetTimer(float detonateDelay, float warnDelay)
{
	m_flDetonateTime = gpGlobals->curtime + detonateDelay;
	m_flWarnAITime = gpGlobals->curtime + warnDelay;
	SetThink(&CGrenadeCstrikeFrag::DelayThink);
	SetNextThink(gpGlobals->curtime);

	//CreateEffects();
}

void CGrenadeCstrikeFrag::OnPhysGunPickup(CBasePlayer* pPhysGunUser, PhysGunPickup_t reason)
{
	...
	if (IsX360())
	{
		...
		SetTimer(2.0f, 1.0f);
		// Removed even if it's for xbox360
		//BlipSound();
		//m_flNextBlipTime = gpGlobals->curtime + FRAG_GRENADE_BLIP_FAST_FREQUENCY;
	}
#endif
...
void CGrenadeCstrikeFrag::DelayThink()
{
...
void CGrenadeCstrikeFrag::SetVelocity(const Vector& velocity, const AngularImpulse& angVelocity)
{
...
int CGrenadeCstrikeFrag::OnTakeDamage(const CTakeDamageInfo& inputInfo)
{
...
bool CGrenadeCstrikeFrag::HandleInteraction(int interactionType, void* data, CBaseCombatCharacter* sourceEnt)
{
...
void CGrenadeCstrikeFrag::InputSetTimer(inputdata_t& inputdata)
{
...
CBaseGrenade* CstrikeFraggrenade_Create(const Vector& position, const QAngle& angles, const Vector& velocity, const AngularImpulse& angVelocity, CBaseEntity* pOwner, float timer, bool combineSpawned)
{
	...
	CGrenadeCstrikeFrag* pGrenade = (CGrenadeCstrikeFrag*)CBaseEntity::Create("npc_grenade_cstrike_frag", position, angles, pOwner);
...
{
	const CGrenadeCstrikeFrag* pFrag = dynamic_cast<const CGrenadeCstrikeFrag*>(pEntity);
	if (pFrag)
	{
