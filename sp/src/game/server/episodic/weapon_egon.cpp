...
ConVar sk_plr_dmg_egon_wide("sk_plr_dmg_egon_wide", 0);
...
void CWeaponEgon::Precache( void )
{
	...
	PrecacheScriptSound("Weapon_Gluon.Ghostbusters");

	PrecacheMaterial("sprites/xbeam1.vmt");
	PrecacheMaterial("sprites/XSpark1.vmt");
...
{
	ConVar* ghostbusters = cvar->FindVar("ghostbusters");
	...
	switch( m_fireState )
	{
		...
		case FIRE_STARTUP:
		{
			...
			{
				if (ghostbusters->GetInt() == 1)
				{
					EmitSound("Weapon_Gluon.Ghostbusters");
				}
				else
				{
...
void CWeaponEgon::EndAttack( void )
{
	...
	if (ghostbusters->GetInt() == 1)
	{
		StopSound("Weapon_Gluon.Ghostbusters");
	}
	else
	{
