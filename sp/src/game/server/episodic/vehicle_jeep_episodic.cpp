...
void CPropJeepEpisodic::Spawn(void)
{
	...
	if (!Q_strnicmp(STRING(gpGlobals->mapname), "d2", 3))
		DestroyHazardLights();
}
...
void CPropJeepEpisodic::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	Vector  vecDelta;
	Vector vecGunPos;
	GetAttachment(LookupAttachment("gun_ref"), vecGunPos);

	VectorSubtract(vecGunPos, tr.endpos, vecDelta);
	distance = VectorNormalize(vecDelta);
	//Now add/remove the gauss gun from the model and give weapon
	if (tr.m_pEnt == this && distance < 8)
	{


		if (m_bHasGun && !pPlayer->Weapon_OwnsThisType("weapon_gauss"))
		{
			SetBodygroup(1, false);
			m_bHasGun = false;

			CPASAttenuationFilter sndFilter(this, "Weapon_Gauss.Dismount");
			EmitSound(sndFilter, entindex(), "Weapon_Gauss.Dismount");

			pPlayer->GiveNamedItem("weapon_gauss");
		}
		else
		{
			CBaseCombatWeapon* pWeapon = pPlayer->Weapon_OwnsThisType("weapon_gauss");
			if (pWeapon)
			{

				CPASAttenuationFilter sndFilter(this, "Weapon_Gauss.Mount");
				EmitSound(sndFilter, entindex(), "Weapon_Gauss.Mount");
...
void CPropJeepEpisodic::Think(void)
{
	...
	if (tr.m_pEnt == this && distance < 8)
	{
		if (m_bHasGun && !pPlayer->Weapon_OwnsThisType("weapon_gauss"))
		{
			UTIL_HudHintText(pPlayer, "#Valve_Hint_GAUSS");
		}

		if (!m_bHasGun && pPlayer->Weapon_OwnsThisType("weapon_gauss"))
		{
			UTIL_HudHintText(pPlayer, "#Valve_Hint_GAUSS2");
		}
