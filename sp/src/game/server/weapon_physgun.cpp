...
//
// Purpose: Thanks to Tingtom for the fix!
//
// $NoKeywords: $
//=============================================================================//
...
ConVar physgun_pellets("physgun_pellets", "0", FCVAR_HIDDEN);
...
public:
	...
	{
		// Want to refer to the Physgun and reduce the pellet count when an object is destroyed! -gaz
		ConVar* pPelletCount = cvar->FindVar("physgun_pellets");
		int i = pPelletCount->GetInt();
		i = i - 1;
		pPelletCount->SetValue(i);
	...
	if (m_align)
	{
		...
		if (len > 0.08)
		{
			...
			if (rotRadius > 0.1)
			{
				...
				/*Uncommented*/	world = m_targetPosition;// + rotDest * (1-ratio);
...
{
	BaseClass::Spawn();
	//	SetModel(GetWorldModel());			//Uncommented
...
void CWeaponGravityGun::AttachObject(CBaseEntity* pObject, const Vector& start, const Vector& end, float distance)
{
	...
	if (pPhysics && pObject->GetMoveType() == MOVETYPE_VPHYSICS)
	{
		...
		//		Msg("Physgun: Object mass: %.2f lbs (%.2f kg)\n", kg2lbs(mass), mass);
...
void CWeaponGravityGun::SecondaryAttack(void)
{
	...
	// Want to refer to the Physgun and reduce the pellet count when an object is destroyed! -gaz
	ConVar* pPelletCount = cvar->FindVar("physgun_pellets");
	int q = pPelletCount->GetInt();
	q = q + 1;
	pPelletCount->SetValue(q);
}
	...
	if (m_active)
	{
		...
		if (pObject && m_glueTouching)
		{
			...
			if (pPellet->MakeConstraint(pObject))
			{
				...
				int i = pPelletCount->GetInt();
				m_pelletCount = i;
				m_activePellets[m_pelletHeld].pellet->Remove();
				m_pelletCount--;
...
void CWeaponGravityGun::ItemPostFrame(void)
{
	...
	//	else if (pOwner->m_afButtonPressed & IN_FIREMODE)
	//	{
	//		Msg("Pellet Count: %i\n", m_pelletCount);
	//	}
		// -----------------------
...
bool CWeaponGravityGun::Reload(void)
{
	...
	// REDO of reload for single pellets? -gaz
	if (i != 0)
	{
		CGravityPellet* pPellet = m_activePellets[i - 1].pellet;

		// There's cases where the gravity pellets aren't killed when they're supposed to be, what to do about this? -gaz
		if (!pPellet)
			return false;
