...
public:
	...
	bool	Reload( void );		//Fix reload sound
...
int detonateCount = 0; //Detonate add ammo
...
void CWeaponStickyLauncher::PrimaryAttack( void )
{
	...
	AddViewKick();
	detonateCount += 1; // detonate add ammo
}

void CWeaponStickyLauncher::SecondaryAttack()
{
	CBasePlayer* pOwner = ToBasePlayer(GetOwner()); // detonate add ammo
	m_flNextSecondaryAttack = gpGlobals->curtime + 1.0;
	pOwner->GiveAmmo(detonateCount, m_iPrimaryAmmoType); //detonate add ammo
	CStickyBomb::DetonateByOperator( GetOwner() );
	detonateCount = 0;
}

bool CWeaponStickyLauncher::Reload(void)		// Fix reload sound
{
...
void CStickyBomb::Touch( CBaseEntity *pOther )
{
	...
	if ( GetMoveType() == MOVETYPE_FLYGRAVITY )
	{
		...
		if (pOther->IsSolidFlagSet(FSOLID_TRIGGER | FSOLID_VOLUME_CONTENTS))	//fix collisions with triggers
		{
			if (pOther->IsSolid())
			{
				// bounce
