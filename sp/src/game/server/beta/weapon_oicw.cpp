...
/*void CWeaponOICW::SecondaryAttack(void)
{
	...
	//Create the grenade
	CGrenadeOICW* pGrenade = (CGrenadeOICW*)Create("grenade_ar2", vecSrc, vec3_angle, pPlayer);
...
void CWeaponOICW::Zoom(void)
{
	...
	color32 lightGreen = { 50, 255, 170, 5 };
...
bool CWeaponOICW::Reload( void )
{
	...
	if (DefaultReload(GetMaxClip1(), GetMaxClip2(), ACT_VM_RELOAD)) //Fix reload sound
		WeaponSound(RELOAD);	//
